...
typedef _CompositionParams = ({
  ...
  int gopSize,
  String creationTime,
  AudioSpoofProfile audioProfile,
  double hueShift,
  double satFactor,
...
typedef _CompositionInputs = ({
  File concatFile,
  List<File> textOverlayFiles,
  bool hasCustomAudio,
  bool hasAmbientAudio,
  int externalAudioCount,
});
...
class VideoBatchComposerImpl implements VideoBatchComposer {
  ...
  Future<ComposerResult> createOutputVideo({
    ...
    final params = _generateParams(config, random);
    ...
    _CompositionInputs? inputs;
    try {
      inputs = await _prepareInputs(outputIndex, segments, config, ambientAudioPath);
      ...
      final List<String> ffmpegArgs = [
        ...
        '-fflags', '+genpts', '-f', 'concat', '-safe', '0', '-i', inputs.concatFile.path,
      ];

      if (inputs.hasCustomAudio) ffmpegArgs.addAll(['-i', config.customAudioPath!]);
      if (inputs.hasAmbientAudio) ffmpegArgs.addAll(['-stream_loop', '-1', '-i', ambientAudioPath!]);

      for (var f in inputs.textOverlayFiles) {
        ffmpegArgs.addAll(['-loop', '1', '-i', f.absolute.path]);
      }
      ...
      // 1. Base Video Filter (GPU Scale -> CPU Crop/Color)
      _appendBaseVideoFilter(filterComplex, engine, params, gpuInfo);

      String lastVideoLabel = '[bg]';
      int overlayIdx = 1;

      // 2. Image Overlays (CPU)
      final imgResult = _appendImageOverlays(filterComplex, config, inputs, lastVideoLabel, overlayIdx, random);
      lastVideoLabel = imgResult.lastLabel;
      overlayIdx = imgResult.nextIdx;

      // 3. Text Overlays (CPU)
      final txtResult = _appendTextOverlays(filterComplex, config, inputs, lastVideoLabel, overlayIdx, random);
      lastVideoLabel = txtResult.lastLabel;
      overlayIdx = txtResult.nextIdx;

      // 4. Audio Mix (CPU)
      _appendAudioMix(filterComplex, config, inputs, params);

      // 5. Finalize Args & Encoding (GPU)
      ffmpegArgs.addAll([
        '-filter_complex', filterComplex.toString(),
        '-map', lastVideoLabel, '-map', '[mixed_a]',
        '-c:a', 'aac', '-b:a', '${params.audioProfile.audioBitrate}k', '-r', '30',
      ]);
      ...
      ffmpegArgs.addAll([
        ...
        '-movflags', '+faststart+use_metadata_tags', '-metadata', 'creation_time=${params.creationTime}', '-avoid_negative_ts', 'make_zero', '-shortest',
        finalOutput
      ]);

      return await _executeFfmpeg(ffmpegArgs, outputIndex, params.targetDuration, finalOutput, context, onProgress);
    } catch (e, st) {
      logs.add('  ❌ Video $outputIndex exception: $e\n$st');
      return (success: false, logs: logs);
    } finally {
      if (inputs != null) {
        if (inputs.concatFile.existsSync()) await inputs.concatFile.delete();
        for (final f in inputs.textOverlayFiles) if (f.existsSync()) await f.delete();
      }
    }
  }

  _CompositionParams _generateParams(BatchVideoConfig config, Random random) {
  ...
  Future<_CompositionInputs> _prepareInputs(
    int outputIndex,
    ...
    return (
      ...
      textOverlayFiles: textOverlayFiles,
      hasCustomAudio: hasCustomAudio,
      hasAmbientAudio: hasAmbientAudio,
      externalAudioCount: externalAudioCount,
    );
  }

  void _appendBaseVideoFilter(
    StringBuffer filterComplex,
    VideoCompositionEngine engine,
    _CompositionParams params,
    GpuInfo gpuInfo,
  ) {
    final bgFilter = engine.buildCompositionFilter(
      ...
      pts: params.pts,
      colorSettings: (
        colorChannelMixer: params.cp?.ffmpegString,
        curves: params.crp?.ffmpegString,
        colorBalance: params.bp?.ffmpegString,
        ...
        satFactor: params.satFactor,
        vignetteAngle: params.vignetteAngle,
      ),
      gpuInfo: gpuInfo,
    );
  ...
  ({String lastLabel, int nextIdx}) _appendImageOverlays(
    StringBuffer filterComplex,
    BatchVideoConfig config,
    _CompositionInputs inputs,
    ...
    int imageInputStartIndex = 1 + inputs.externalAudioCount + config.textOverlays.length;
    String currentLastLabel = lastVideoLabel;
    int currentIdx = overlayIdx;
    ...
    for (int i = 0; i < config.imageOverlays.length; i++) {
      ...
      String scaleLabel = '[scaled$currentIdx]';
      ...
      String nextVideoLabel = '[ov$currentIdx]';
      filterComplex.write('$currentLastLabel$scaleLabel' 'overlay=${targetX + jX}:${targetY + jY}:enable=\'between(t,${imgConfig.startTime},${imgConfig.endTime ?? 99999})\'${imgConfig.isGif ? ":shortest=1" : ""}$nextVideoLabel;');
      currentLastLabel = nextVideoLabel;
      currentIdx++;
    }
    return (lastLabel: currentLastLabel, nextIdx: currentIdx);
  }

  ({String lastLabel, int nextIdx}) _appendTextOverlays(
    StringBuffer filterComplex,
    BatchVideoConfig config,
    _CompositionInputs inputs,
    String lastVideoLabel,
    ...
    for (int i = 0; i < config.textOverlays.length; i++) {
      ...
      int textInputIdx = 1 + inputs.externalAudioCount + i;
      if (!overlay.isAnimated) {
        ...
        final nextVideoLabel = '[ov$currentIdx]';
        filterComplex.write('$currentLastLabel$label' 'overlay=x=\'$tJX+1.0*sin(2*PI*n/15)\':y=\'$tJY+1.0*cos(2*PI*n/15)\':enable=\'between(t,${overlay.startTime},${overlay.endTime ?? 99999})\':shortest=1$nextVideoLabel;');
        currentLastLabel = nextVideoLabel;
        currentIdx++;
      } else {
        ...
        final nextVideoLabel = '[ov$currentIdx]';
        filterComplex.write('$currentLastLabel$label' 'overlay=x=\'$xE+1.0*sin(2*PI*n/20)\':y=\'$yE+1.0*cos(2*PI*n/20)\':enable=\'between(t,$start,$end)\':shortest=1$nextVideoLabel;');
        currentLastLabel = nextVideoLabel;
        currentIdx++;
      }
    }
  ...
  void _appendAudioMix(
    StringBuffer filterComplex,
    BatchVideoConfig config,
    _CompositionInputs inputs,
    _CompositionParams params,
  ) {
    if (inputs.hasCustomAudio) {
      filterComplex.write('[0:a]${params.audioProfile.toOriginalAudioFilterChain(volume: 0.25, pts: params.pts)}[orig_a];');
      filterComplex.write('[1:a]${params.audioProfile.toCustomAudioFilterChain(volume: config.customAudioVolume.clamp(0.0, 1.0), pts: params.pts)}[music_a];');
    } else {
      filterComplex.write('[0:a]${params.audioProfile.toOriginalAudioFilterChain(volume: 0.05, pts: params.pts)}[orig_a];');
    }
    String mixLabels = '[orig_a]';
    int mixInputs = 1;
    if (inputs.hasCustomAudio) { mixInputs++; mixLabels += '[music_a]'; }
    if (inputs.hasAmbientAudio) {
      mixInputs++;
      final ambientInputIdx = inputs.hasCustomAudio ? 2 : 1;
      filterComplex.write('[$ambientInputIdx:a]volume=${(inputs.hasCustomAudio ? 0.25 : 0.05).toStringAsFixed(3)},aresample=44100,aformat=channel_layouts=stereo[ambient_a];');
      mixLabels += '[ambient_a]';
    }
    if (mixInputs > 1) {
