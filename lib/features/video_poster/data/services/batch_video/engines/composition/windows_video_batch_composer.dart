...
@LazySingleton(as: VideoBatchComposer, env: ['windows'])
class WindowsVideoBatchComposer implements VideoBatchComposer {
  final VideoHardwareCapabilityResolver _hardwareResolver;

  WindowsVideoBatchComposer(this._hardwareResolver);
  ...
  Future<ComposerResult> createOutputVideo({
    ...
    try {
      ...
      final List<String> ffmpegArgs = [
        ...
        if (isNvidia && gpuInfo.hasCudaFilters) ...['-hwaccel_output_format', 'cuda'] 
        else if (gpuInfo.hwaccel != null && gpuInfo.outputFormat != null) ...['-hwaccel_output_format', gpuInfo.outputFormat!],
      ...
      // Step 1: Scale + crop (Windows hardware acceleration logic)
      if (isNvidia && gpuInfo.hasCudaFilters) {
        filterComplex.write('[0:v]scale_cuda=$zoomW:$zoomH,crop=1080:1920:(iw-1080)*$randX:(ih-1920)*$randY,hwdownload,format=nv12,');
      } else if (gpuInfo.scaleFilter != null && gpuInfo.scaleFilter != 'scale') {
        filterComplex.write('[0:v]${gpuInfo.scaleFilter!}=$zoomW:$zoomH,${gpuInfo.outputFormat != null ? 'hwdownload,format=nv12,' : ''}');
      ...
      filterComplex.write('${finalFormat}trim=start=0,setpts=${pts.toStringAsFixed(6)}*N/30/TB[bg];');

      String lastVideoLabel = '[bg]';
      int overlayIdx = 1;
      int imageInputStartIndex = 1 + externalAudioCount + config.textOverlays.length;

      // Image Overlays (Same logic as Mac implementation)
      for (int i = 0; i < config.imageOverlays.length; i++) {
        ...
        int finalW = targetW; int finalH = targetH;
      ...
      String mixLabels = '[orig_a]'; int mixInputs = 1;
      ...
      if (gpuInfo.encoder == 'libx264') {
        ffmpegArgs.addAll(['-preset', 'superfast', '-g', gopSize.toString()]);
      } else if (gpuInfo.encoder == 'h264_qsv') {
        ffmpegArgs.addAll(['-preset', 'veryfast', '-g', gopSize.toString()]);
      } else if (gpuInfo.encoder == 'h264_nvenc') {
        ffmpegArgs.addAll(['-preset', 'p1', '-tune', 'hq', '-g', gopSize.toString()]);
      } else {
        ffmpegArgs.addAll(['-g', gopSize.toString()]);
      }
