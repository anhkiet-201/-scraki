...
class VtToolkit extends BaseFfmpegToolkit {
  ...
  String buildOverlayChain(CompositionPlan plan, String inputLabel) {
    ...
    for (var i = 0; i < config.imageOverlays.length; i++) {
      ...
      String scaleF =
          '[$currentInputIdx:v]${scale(fTargetW, fTargetH)},format=yuva420p,${eq(brightness: oBright, saturation: oSat)}';
    ...
    for (var i = 0; i < plan.textOverlayPaths.length; i++) {
      ...
      if (!overlayCfg.isAnimated) {
        ...
        sb.write(
          '[$textInputIdx:v]${scale(0, 0, iwScale: tScale)},format=rgba,${rotate(tRotate)},colorchannelmixer=aa=$tOpacity$label;',
        );

        final nextLabel = '[v_ov${overlayIdx++}]';
        sb.write(
          '$lastLabel$label'
          '${overlay(x: '$tJX+1.0*sin(2*PI*n/15)', y: '$tJY+1.0*cos(2*PI*n/20)', enable: 'between(t,${overlayCfg.startTime},${overlayCfg.endTime ?? 99999})')}$nextLabel;',
        );
        lastLabel = nextLabel;
      } else {
        final int tW = (overlayCfg.width * 1.5).round();
