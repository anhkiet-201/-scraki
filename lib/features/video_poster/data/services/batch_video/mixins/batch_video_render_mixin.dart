  ...
  Future<({bool success, List<String> logs})> createOutputVideo({
    ...
    final concatFile = File(
      p.join(
        ...
        'scraki_concat_${outputIndex}_${DateTime.now().millisecondsSinceEpoch}.txt',
      ),
    );
    ...
    final filterId = colorProfile != null
        ? '${colorProfile.type}_${curvesProfile?.type}'
        : 'none';
    final finalOutput =
    ...
    try {
      ...
      final List<String> ffmpegArgs = [
        ...
        // Giải mã bằng GPU (videotoolbox), sau đó do dùng CPU filter (scale, crop)
        ...
        if (isNvidia && gpuInfo.hasCudaFilters) ...[
          '-hwaccel_output_format',
          'cuda',
        ] else if (gpuInfo.hwaccel != null && gpuInfo.outputFormat != null) ...[
          '-hwaccel_output_format',
          gpuInfo.outputFormat!,
        ],
        '-fflags', '+genpts',
      ...
      final hasAmbientAudio =
          ambientAudioPath != null && File(ambientAudioPath).existsSync();
      if (hasAmbientAudio) {
        ffmpegArgs.addAll(['-stream_loop', '-1', '-i', ambientAudioPath]);
      }

      final int externalAudioCount =
          (hasCustomAudio ? 1 : 0) + (hasAmbientAudio ? 1 : 0);
      ...
      for (var i = 0; i < config.textOverlays.length; i++) {
        ...
        final file = File(
          p.join(
            Directory.systemTemp.path,
            'scraki_text_${outputIndex}_${i}_${DateTime.now().millisecondsSinceEpoch}.png',
          ),
        );
      ...
      // ── Bước 1: Scale + crop ────────────────────────────────────────────────
      // Strategy: Giữ frame trên GPU càng lâu càng tốt trước khi download cho CPU filters.
      // Thứ tự ưu tiên:
      //   1. NVIDIA CUDA: scale_cuda + crop → hwdownload (1 lần duy nhất) → CPU color filters
      //   2. macOS VT: scale_vt → CPU color filters (~0% CPU cho scale)
      //   3. QSV: vpp_qsv scale → CPU color filters
      //   4. Software fallback
      if (isNvidia && gpuInfo.hasCudaFilters) {
        // NVIDIA: Dùng scale_cuda, giữ trên GPU. crop chạy trên CUDA frame (supported).
        // hwdownload chỉ xảy ra 1 lần sau crop, trước CPU color filters.
        filterComplex.write(
          '[0:v]scale_cuda=$zoomW:$zoomH,crop=1080:1920:(iw-1080)*$randX:(ih-1920)*$randY,hwdownload,format=nv12,',
        );
      } else if (gpuInfo.scaleFilter != null &&
          gpuInfo.scaleFilter != 'scale') {
        // Windows QSV hoặc các hardware scaler khác (không phải CUDA)
        final hwScale = gpuInfo.scaleFilter!;
        final downloadCmd = gpuInfo.outputFormat != null
            ? 'hwdownload,format=nv12,'
            : '';
        filterComplex.write('[0:v]$hwScale=$zoomW:$zoomH,$downloadCmd');
        filterComplex.write(
          'crop=1080:1920:(iw-1080)*$randX:(ih-1920)*$randY,',
        );
      } else {
        // Software fallback: tất cả CPU
        filterComplex.write(
      ...
      // ── Bước 2: CPU Color filters ───────────────────────────────────────────
      // Tất cả các filter này chạy trên CPU (FFmpeg chưa có GPU-native equivalent).
      // Nhưng chúng chạy trên frame đã được scale_cuda/scale_vt xử lý xuống 1080p,
      // nên CPU load thấp hơn nhiều so với chạy trên source 4K.
      if (config.generateColorFilter &&
          colorProfile != null &&
          curvesProfile != null) {
        ...
        filterComplex.write(
          'eq=brightness=${brightness.toStringAsFixed(4)}:contrast=${contrast.toStringAsFixed(4)}:gamma_r=${gammaR.toStringAsFixed(3)}:gamma_g=${gammaG.toStringAsFixed(3)}:gamma_b=${gammaB.toStringAsFixed(3)},',
        );
      }

      filterComplex.write(
        'hue=h=${hueShift.toStringAsFixed(2)}:s=${satFactor.toStringAsFixed(4)},',
      );
      filterComplex.write('vignette=${vignetteAngle.toStringAsFixed(4)},');

      // Đảm bảo pixel format đúng trước khi vào encoder
      final String finalFormat = isNvidia ? 'format=nv12,' : 'format=yuv420p,';
      filterComplex.write(
        '${finalFormat}trim=start=0,setpts=$ptsStr*N/30/TB[bg];',
      );

      int overlayIdx = 1;
      String lastVideoLabel = '[bg]';
      int imageInputStartIndex =
          1 + externalAudioCount + config.textOverlays.length;
      ...
      for (int i = 0; i < config.imageOverlays.length; i++) {
        ...
        String scaleFilter =
            '[$currentInputIdx:v]scale=$finalTargetW:$finalTargetH,format=rgba,eq=brightness=$overlayBright:saturation=$overlaySat';
        ...
        if (imgConfig.borderWidth > 0) {
          final String borderHex = _colorToHex(
            imgConfig.borderColor ?? Colors.white,
          );
        ...
        String enableFilter =
            "enable='between(t,${imgConfig.startTime},${imgConfig.endTime ?? 99999})'";
      ...
      for (int i = 0; i < config.textOverlays.length; i++) {
        ...
        if (!overlay.isAnimated) {
          ...
          filterComplex.write(
            '[$textInputIdx:v]scale=iw*$textScale:-1,format=rgba,rotate=$textRotate*PI/180:c=black@0,colorchannelmixer=aa=$textOpacity$antiOcrLabel;',
          );
          String enableFilter =
              "enable='between(t,${overlay.startTime},${overlay.endTime ?? 99999})'";
          ...
          String filterBlock =
              '[$textInputIdx:v]scale=$targetW:$targetH,format=rgba';
          if (overlay.rotation != 0)
            filterBlock +=
                ',rotate=${overlay.rotation}*PI/180:c=black@0:ow=$finalW:oh=$finalH';
          ...
          if (overlay.animationOutType != 'none' && totalDur > 0) {
            ...
            if (overlay.animationOutType == 'fade')
              ...
              scaleExpr =
                  'if(gt(t,$startOut),1.0-(t-$startOut)/$durOut,$scaleExpr)';
          ...
          if (scaleExpr != '1.0')
            filterBlock +=
                ",scale='bitand(iw*$scaleExpr,-2)':'bitand(ih*$scaleExpr,-2)':eval=frame";
          String finalXExpr = '$targetX';
          if (xExprIn.isEmpty && xExprOut.isNotEmpty)
            finalXExpr =
                'if(gt(t\\,${end - (totalDur * overlay.animationOutDuration)})\\,$xExprOut\\,$targetX)';
          else if (xExprIn.isNotEmpty && xExprOut.isEmpty)
            finalXExpr =
                'if(lt(t\\,${start + (totalDur * overlay.animationInDuration)})\\,$xExprIn\\,$targetX)';
            ...
            finalXExpr =
                'if(lt(t\\,$endIn)\\,$xExprIn\\,if(gt(t\\,$startOut)\\,$xExprOut\\,$targetX))';
          }
          String finalYExpr = '$targetY';
          if (yExprIn.isEmpty && yExprOut.isNotEmpty)
            finalYExpr =
                'if(gt(t\\,${end - (totalDur * overlay.animationOutDuration)})\\,$yExprOut\\,$targetY)';
          else if (yExprIn.isNotEmpty && yExprOut.isEmpty)
            finalYExpr =
                'if(lt(t\\,${start + (totalDur * overlay.animationInDuration)})\\,$yExprIn\\,$targetY)';
          else if (yExprIn.isNotEmpty && yExprOut.isNotEmpty) {
            final endIn = start + (totalDur * overlay.animationInDuration);
            final startOut = end - (totalDur * overlay.animationOutDuration);
            finalYExpr =
                'if(lt(t\\,$endIn)\\,$yExprIn\\,if(gt(t\\,$startOut)\\,$yExprOut\\,$targetY))';
          }
          ...
          filterComplex.write(
            ...
            'overlay=x=\'$finalXExpr+$driftX\':y=\'$finalYExpr+$driftY\':enable=\'between(t,$start,$end)\':shortest=1$nextVideoLabel;',
          );
          lastVideoLabel = nextVideoLabel;
          overlayIdx++;
        }
      ...
      if (hasCustomAudio) {
        ...
        final customChain = audioProfile.toCustomAudioFilterChain(
          volume: customVol,
          pts: pts,
        );
      ...
      if (hasAmbientAudio) {
        ...
        fStr +=
            '[$ambientInputIdx:a]volume=${ambientVol.toStringAsFixed(3)},aresample=44100,aformat=channel_layouts=stereo[ambient_a];';
        mixLabels += '[ambient_a]';
      }
      if (mixInputs > 1) {
        fStr +=
      ...
      // Fix Bug 4: dùng preferredPixFmt phù hợp với encoder thay vì hardcode
      final String outputPixFmt = gpuInfo.preferredPixFmt;
      ...
      ffmpegArgs.addAll([
        ...
        'aac',
        '-b:a',
        '${audioProfile.audioBitrate}k',
        '-r',
        '30',
        '-c:v',
        gpuInfo.encoder,
        '-b:v',
        '${10 + random.nextInt(6)}M',
        '-maxrate',
        '16M',
        '-bufsize',
        '25M',
      ...
      if (!success) {
        ...
        logs.add(
          '  ❌ Video $outputIndex thất bại (exit=$exitCode)${errorLines.isNotEmpty ? ':\n$errorLines' : '.'}\n$filterInfo',
        );
      }
